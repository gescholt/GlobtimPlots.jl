...
@enum PlotBackend begin
    Interactive  # GLMakie for interactive plots (requires `using GLMakie`)
...
"""
    generate_experiment_labels(campaign) -> Vector{String}
...
# Arguments
- `campaign`: Campaign results object with `.experiments` field
...
"""
function generate_experiment_labels(campaign)
    labels = String[]
...
"""
    create_experiment_plots(result, stats::Dict; backend::PlotBackend=Static) -> Figure
...
"""
    create_single_plot(result, stats::Dict, plot_type::Symbol, title::String; backend::PlotBackend=Interactive) -> Figure
...
"""
    create_campaign_comparison_plot(campaign, stats::Vector; backend::PlotBackend=Static) -> Figure or Vector{Figure}
