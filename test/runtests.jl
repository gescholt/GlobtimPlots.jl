...
function _make_capture_result(
    ...
    for sym in unique(types)
        ...
        type_rates[sym] =
            [count(captured[ti][idxs]) / length(idxs) for ti in 1:length(tol_fracs)]
    end
...
@testset "GlobtimPlots.jl" begin
    ...
    @testset "plot_capture_convergence" begin
        ...
        @testset "Error on empty input" begin
            known =
                KnownCriticalPoints([[0.0, 0.0]], [0.0], [:min], [(-1.0, 1.0), (-1.0, 1.0)])
        ...
        @testset "Single degree with support_sizes" begin
            types = [:min]
            known =
                KnownCriticalPoints([[0.5, 0.5]], [1.0], types, [(0.0, 1.0), (0.0, 1.0)])
            dd = known.domain_diameter
            cr = _make_capture_result([0.1], types, [0.05], dd)

            fig = plot_capture_convergence([(6, cr)], known; support_sizes = Dict(6 => 28))
            @test fig isa Figure
        end
    end
    ...
    @testset "plot_capture_sparsification_combined" begin
        ...
        function _make_combined_entries(degrees, variant_specs, types, known)
            ...
            for deg in degrees
                for (vlabel, threshold, dist_scale, n_nnz, l2r) in variant_specs
                    ...
                    base_time = deg^2 * 0.1
                    solve_t =
                        threshold > 0.0 ? base_time / (1.0 + 10.0 * threshold) : base_time
                    push!(
                        entries,
                        (
                            degree = deg,
        ...
        @testset "Single degree, single variant (Full only)" begin
            ...
            entries =
                _make_combined_entries([4], [("Full", 0.0, 1.0, 100, 1.0)], types, known)
        ...
        @testset "3 types (min, max, saddle)" begin
            ...
            variant_specs = [("Full", 0.0, 1.0, 200, 1.0), ("1e-4", 1e-4, 1.3, 150, 0.95)]
